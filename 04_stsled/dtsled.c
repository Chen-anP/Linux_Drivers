...
static int __init led_init(void)
{
    ...
    //查找设备节点
    dtsled.nd = of_find_node_by_path("/rk3588_led");
    if(dtsled.nd == NULL){
	...
	/* 1、寄存器地址映射 */
	// led_remap();
    PMU2_IOC_GPIO0C_IOMUX_SEL_H_VA = of_iomap(dtsled.nd, 0);
    
    GPIO_SWPORT_DR_H_VA = of_iomap(dtsled.nd, 1);
    GPIO_SWPORT_DDR_H_VA = of_iomap(dtsled.nd, 2);
